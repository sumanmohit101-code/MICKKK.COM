...
let watchQuoteCache = {};
let watchQuoteRequestVersion = 0;
...
let volMAConfig = { enabled: true, len: 30, color: '#ff9800', width: 1 };
...
let smartBarConfig = {
  enabled: true, bodyEnabled: true, bodyThreshold: 4.0, bodyColor: '#2196f3',
  rangeEnabled: true, rangeThreshold: 3.0, rangeColor: '#ffffff',
  nr7Enabled: true, nr7Color: '#ff9900'
};

let ibMaxMotherBarRange = 8.0;
let mcpConfig = { enabled: true, r1: 8.0, lastOnly: false };
let gapConfig = { enabled: true, minGapPct: 1.0, maxGaps: 5 };
let atrExtConfig = { enabled: true, threshold: 5.0 };

let activeIndicators = {
  ppv: true, tables: true, tablesEmaDist: false,
  candleClr: true, ibLabel: true, wtc: true, ema9Sell: true,
  sellClimax: true, buyClimax: true
};
...
function updateTimeScaleVisibility(panel) {
  ...
  const splitter = document.getElementById(`splitter-1-${panel.index}`);
  const isDark = currentTheme === 'dark';

  if (paneRsi) paneRsi.style.display = showRsi ? 'block' : 'none';
  if (splitter) splitter.style.display = showRsi ? 'block' : 'none';

  panel.priceChart.timeScale().applyOptions({
    visible: !showRsi,
    height: showRsi ? 0 : 28,
    timeVisible: false,
    secondsVisible: false,
    ticksVisible: !showRsi,
  ...
  panel.rsiChart.timeScale().applyOptions({
    visible: showRsi,
    height: showRsi ? 28 : 0,
    timeVisible: false,
    secondsVisible: false,
...
function createPanelChartInstance(index, tf) {
  ...
  rsiC.subscribeCrosshairMove(param => {
    ...
    syncCrosshairAtX(rsiC, priceC, param, instObj.candleSeries, targetTime => {
  ...
  syncPanes(priceC); syncPanes(rsiC);
...
function toggleTheme() {
  ...
  const isDark = currentTheme === 'dark';
  panelsArray.forEach(p => {
    [p.priceChart, p.rsiChart].forEach(c => {
      if (c) {
        c.applyOptions({
...
function renderWatchlists(){
  ...
  (wl.sections || []).forEach(section => {
    ...
    if (!collapsed) {
      ...
      (section.items || []).forEach(item => {
        ...
        const q = watchQuoteCache[sym] || {};
        const ltp = Number(q.ltp);
        const chg = Number(q.changePct);
        const ltpText = Number.isFinite(ltp) && ltp > 0 ? ltp.toFixed(2) : '—';
        const chgText = Number.isFinite(chg) ? `${chg >= 0 ? '+' : ''}${chg.toFixed(2)}%` : '—';
        const chgCls = Number.isFinite(chg) ? (chg >= 0 ? 'pos' : 'neg') : '';
        if (!watchQuoteCache[sym] || !watchQuoteCache[sym].loaded) symbolsToQuote.push(sym);
        const row = document.createElement('div');
        row.className = 'watch-item' + (sym === currentSymbol ? ' active' : '');
        row.innerHTML = `<span class="watch-sym">${escapeWatchText(sym)}</span><span class="watch-name">${escapeWatchText(item.name || '')}</span><span class="watch-quote"><span class="watch-ltp">${ltpText}</span><span class="watch-daychg ${chgCls}">${chgText}</span></span><button class="watch-remove" title="Remove from section">×</button>`;
        row.onclick = () => loadSymbol(sym, item.name || sym, -1);
        row.querySelector('.watch-remove').onclick = async (e) => { e.stopPropagation(); await mutateWatchlist('removeSymbol', {watchlist: wl.name, section: section.name, symbol: sym}); };
  ...
  if (symbolsToQuote.length) refreshWatchQuotes([...new Set(symbolsToQuote)]);
}

async function fetchWatchQuote(symbol){
  const sym = String(symbol || '').trim().toUpperCase();
  if (!sym) return null;
  try {
    const liveRes = await fetch(`${CHARTS_API_URL}?action=getLive&symbol=${encodeURIComponent(sym)}`, { cache: 'no-store' });
    const live = await liveRes.json();
    const ltp = Number(live && live.price);
    let prevClose = NaN;
    const candles = await fetchSymbolCandles(sym);
    if (Array.isArray(candles) && candles.length >= 2) {
      const last = candles[candles.length - 1];
      const prev = candles[candles.length - 2];
      // If today's candle exists, compare LTP with yesterday's close; otherwise latest close with previous close.
      const today = new Date().toISOString().slice(0,10);
      if (String(last.time).slice(0,10) === today) prevClose = Number(prev.close);
      else prevClose = Number(last.close);
    }
    const price = Number.isFinite(ltp) && ltp > 0 ? ltp : (candles && candles.length ? Number(candles[candles.length-1].close) : NaN);
    const changePct = Number.isFinite(price) && Number.isFinite(prevClose) && prevClose > 0 ? ((price - prevClose) / prevClose) * 100 : NaN;
    return { ltp: price, changePct, loaded: true, ts: Date.now() };
  } catch(e) {
    return { loaded: true, error: e.message || String(e), ts: Date.now() };
  }
}

async function refreshWatchQuotes(symbols){
  const version = ++watchQuoteRequestVersion;
  const unique = [...new Set(symbols.map(s => String(s).toUpperCase()).filter(Boolean))];
  await Promise.all(unique.map(async sym => {
    const q = await fetchWatchQuote(sym);
    if (q) watchQuoteCache[sym] = q;
  }));
  if (version === watchQuoteRequestVersion) renderWatchlists();
}
...
async function readCandleCache(symbol){
  try {
    const db = await openCandleCacheDB();
    if (!db) return null;
    return await new Promise(resolve => {
      const tx = db.transaction(CANDLE_DB_STORE, 'readonly');
      const req = tx.objectStore(CANDLE_DB_STORE).get(symbol);
...
async function writeCandleCache(symbol, candles){
  try {
    const db = await openCandleCacheDB();
    if (!db) return;
...
async function refreshSymbolCandlesInBackground(symbol){
  if (symbolCandleRefreshPromise[symbol]) return symbolCandleRefreshPromise[symbol];
  symbolCandleRefreshPromise[symbol] = (async () => {
    try {
      const candles = await fetchFreshCandlesFromServer(symbol);
      if (!candles.length) return;
      symbolCandleCache[symbol] = candles;
      symbolCandleCacheMeta[symbol] = Date.now();
      try { sessionStorage.setItem(`MICKKK_CANDLES_${symbol}`, JSON.stringify(candles)); } catch(e){}
      await writeCandleCache(symbol, candles);
    } catch(e) {
      // Background refresh must never disturb the already-rendered chart.
    } finally {
      delete symbolCandleRefreshPromise[symbol];
    }
  })();
  return symbolCandleRefreshPromise[symbol];
}

async function fetchSymbolCandles(symbol) {
  if (symbolCandleCache[symbol]) {
    const savedAt = symbolCandleCacheMeta[symbol] || 0;
    if (Date.now() - savedAt > CANDLE_CACHE_TTL_MS) {
      refreshSymbolCandlesInBackground(symbol);
    }
    return symbolCandleCache[symbol];
  }
  if (symbolCandlePromiseCache[symbol]) return symbolCandlePromiseCache[symbol];

  // 1) Persistent browser cache: fastest path on repeat visits/reloads.
  try {
    const local = await readCandleCache(symbol);
    if (local && Array.isArray(local.candles) && local.candles.length) {
      symbolCandleCache[symbol] = local.candles;
      symbolCandleCacheMeta[symbol] = Number(local.savedAt || 0);
      if (Date.now() - symbolCandleCacheMeta[symbol] > CANDLE_CACHE_TTL_MS) {
        refreshSymbolCandlesInBackground(symbol);
      }
  ...
  // 2) Keep the existing sessionStorage cache as a compatibility fallback.
  try {
    const cached = sessionStorage.getItem(`MICKKK_CANDLES_${symbol}`);
    if (cached) {
      const parsed = JSON.parse(cached);
      if (Array.isArray(parsed) && parsed.length) {
        symbolCandleCache[symbol] = parsed;
        symbolCandleCacheMeta[symbol] = Date.now();
        writeCandleCache(symbol, parsed);
        return parsed;
      }
    }
  } catch(e){}

  // 3) First-ever load: fetch from Google Sheet/API, then persist locally.
  ...
  try { return await symbolCandlePromiseCache[symbol]; }
  finally { delete symbolCandlePromiseCache[symbol]; }
}
...
function processCandleColoringAndIB(data, panel, tf) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (customColor) { cObj.color = customColor; }
    candles.push(cObj);
  ...
  emaConfigs.forEach(cfg => {
    if (cfg.enabled) {
      ...
      const s = panel.priceChart.addLineSeries({
        color: cfg.color, lineWidth: cfg.width, priceLineVisible: false, lastValueVisible: false, crosshairMarkerVisible: false
      });
      s.setData(emaData);
...
function resetToDefaults(){
  ...
  document.getElementById('candle-clr-en').checked = true;
  document.getElementById('smart-body-en').checked = true;
  document.getElementById('smart-range-en').checked = true;
  document.getElementById('smart-nr7-en').checked = true;
  document.getElementById('ib-label-en').checked = true;
  document.getElementById('mcp-en').checked = true;
  ...
  document.getElementById('atr-ext-en').checked = true;
  document.getElementById('volma-en').checked = true;
  document.getElementById('gap-en').checked = true;
  document.getElementById('wtc-en').checked = true;
  document.getElementById('ema9sell-en').checked = true;
  document.getElementById('sellclimax-en').checked = true;
  document.getElementById('buyclimax-en').checked = true;
  ...
  updateEMAConfig(); updateSmartBarConfig(); updateVolMAConfig(); updateRSIConfig(); updateGapConfig(); updateMCPConfig(); updateATRExtConfig();
}

/* ============ UPCOMING RESULT DATE ============ */
let upcomingResultCache = {};
let resultFetchInFlight = {};

function formatResultDate(v){
  if (!v) return '—';
...
function pickResultDate_(obj){
  if (!obj || typeof obj !== 'object') return '';
  const keys = Object.keys(obj);
  const preferred = ['nextResultDate','upcomingResultDate','next_result_date','upcoming_result_date','nextDate','resultDate','result_date','Next Result Date','Upcoming Result Date'];
  for (const k of preferred) if (obj[k]) return obj[k];
  for (const k of keys) if (/next.*result.*date|upcoming.*result.*date|result.*date/i.test(k) && obj[k]) return obj[k];
  return '';
}

async function fetchUpcomingResultDate(symbol){
  const sym = String(symbol || '').trim().toUpperCase();
  if (!sym) return '';
  if (upcomingResultCache[sym] !== undefined) return upcomingResultCache[sym];
  if (resultFetchInFlight[sym]) return resultFetchInFlight[sym];
  resultFetchInFlight[sym] = (async () => {
    try {
      // Preferred endpoint: a dedicated Results/Results Sheet endpoint if present.
      for (const action of ['getResults','getEarnings']) {
        try {
          const res = await fetch(`${CHARTS_API_URL}?action=${action}&symbol=${encodeURIComponent(sym)}`, { cache:'no-store' });
          if (!res.ok) continue;
          const payload = await res.json();
          let date = pickResultDate_(payload);
          if (!date && payload && payload.result) date = pickResultDate_(payload.result);
          if (!date && payload && payload.results) {
            if (Array.isArray(payload.results)) {
              const row = payload.results.find(r => String(r.symbol || r.Symbol || '').toUpperCase() === sym) || payload.results[0];
              date = pickResultDate_(row);
            } else date = pickResultDate_(payload.results);
          }
          if (date) { upcomingResultCache[sym] = date; return date; }
        } catch(e) {}
      }
    } catch(e) {}
    upcomingResultCache[sym] = '';
    return '';
  })();
  try { return await resultFetchInFlight[sym]; }
  finally { delete resultFetchInFlight[sym]; }
}

function refreshUpcomingResultDate(){
  const sym = currentSymbol;
  if (!sym) return;
  fetchUpcomingResultDate(sym).then(date => {
...
function renderCombinedInfoCard(data){
  ...
  try {
    ...
    infoCard.innerHTML = `
      ...
      <div class="info-card-divider"></div>
      <div class="info-card-section">
        <div class="info-row result-date-row"><span>Upcoming Result:</span> <span class="result-date-value">${formatResultDate(upcomingResultCache[currentSymbol.toUpperCase()] || '')}</span></div>
      </div>
      <div class="info-card-divider"></div>
...
function applyRangeToPanel(panel, data) {
  ...
  if (currentRange === 'ALL') {
    [panel.priceChart, panel.rsiChart].forEach(c => { try { c.timeScale().fitContent(); } catch(e){} });
    return;
  }
  ...
  try {
    ...
    [panel.priceChart, panel.rsiChart].forEach(c => {
