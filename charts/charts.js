...
async function loadSymbolList(){
  let first = null;
  try{
    const res = await fetch(`${CHARTS_API_URL}?action=getSymbols&_ts=${Date.now()}`, { cache: 'no-store' });
    const data = await res.json();
    if (!data.error) {
      const fetched = Array.isArray(data.symbols) ? data.symbols : [];
      if (fetched.length) allSymbols = fetched;
    }
  }catch(e){
    console.warn('Symbol list fetch failed; using fallback list:', e);
  }

  first = allSymbols[0];
  if (first) {
    try {
      await loadSymbol(first.symbol, first.name, -1);
    } catch(e) {
      showChartError('Chart startup failed: ' + (e && e.message ? e.message : e));
    }
  } else {
    showChartError('No stock symbols available. Check the Google Apps Script deployment.');
  }
...
async function loadSymbol(symbol, name, targetIdx){
  ...
  let hasCache = false;
  try { hasCache = !!localStorage.getItem(`MICKKK_CANDLES_DATA_${symbol}`); } catch(e) {}
  if (!hasCache) {
...
async function rebuildPanelChart(panel) {
  ...
  try {
    ...
    if (panel && panel.index === 0) {
      showChartError(err && err.message ? err.message : String(err));
    }
    throw err;
  }
}
